...
<script>
	...
	export default{
		...
		computed:{
			income_id(){
				return this.$store.state.finance.income_id
			}
		},
    mounted() {
      var _self = this
      //账目分类
      axios_post('/api/ac/list',{offset:1,limit:50,type:'1',status:'1',flag:'1',}).then(res => {
      	if(res){
          _self.categoryList = res.records
      	}
      })
      //该账目详情
      axios_post('/api/fi/inout/detail/'+_self.income_id).then(res => {
        if(res){
      ...
      handleEdit:function(){
        ...
        _self.$refs['ruleForm'].validate((valid) => {
          var req = JSON.parse(JSON.stringify(_self.info)) //深拷贝
            ...
            _self.$confirm('确认保存修改信息?', '提示').then(()=> {
              axios_post('/api/fi/inout/save',req).then( res => {
                if(res===null){
                  _self.$router.push("/finance/fin/income")
                }
              })
            })
        	}else{
          ...
          this.$router.push('/finance/fin/income')
        })
			}
		}
	}
