  ...
  <div>
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-form-item label="账目分类">
        <el-select v-model="search_info.categoryId">
          ...
          <el-option v-for="n in categoryList" :label="n.name" :value="n.id" :key="n.id"></el-option>
        </el-select>
      </el-form-item>
    ...
    <el-table :data="list" border style="width: 100%">
      ...
      <el-table-column label="入账时间">
        <template slot-scope="scope">{{time_format(scope.row.payDate)}}</template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    leadingOut() {
      ...
      axios_post("/api/fi/inout/list",xxx).then(res => {
        if (res) {
          ...
          const tHeader = ['账目编号', '账目名称','账目分类','交易对象','金额(元)','出账时间','清账状态'] // 对应表格输出的title
          const filterVal = ['id','name','categoryName','trader','totalAmount','time','status'] // 对应表格输出的数据
          ...
          export_json_to_excel(tHeader, data, '入账管理') // 对应下载文件的名字
        })
        }
      });
    ...
    handleAdd: function() {
      this.$router.push("/finance/fin/addIncome");
    },
    handleEdit: function(_id) {
      var _self = this;
      _self.$store.commit("finance_income", { _id: _id });
      _self.$router.push("/finance/fin/incomeDetail");
    },
