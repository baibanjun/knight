	...
	<div>
      ...
      <el-form-item label="清账状态">
      	<el-select v-model="search_info.status">
      		<el-option label="全部" value="0"></el-option>
          <el-option label="已付账" value="3"></el-option>
      ...
      <el-table-column label="出账时间"><template slot-scope="scope">{{time_format(scope.row.payDate)}}</template> </el-table-column>
...
<script>
  ...
  import { time_format } from '@/lib/tools.js'
	export default {
		...
		created() {
      var _self = this
      axios_post('/api/ac/list',{offset:1,limit:10,type:'2',status:'1',flag:'0',}).then(res => {
      	if(res){
    ...
    leadingOut() {
      var xxx={
          ...
          date:this.search_info.date,
        	categoryId:this.search_info.categoryId,
          status:this.search_info.status,
        	type:'2'
        }
        axios_post('/api/fi/inout/list',xxx).then(res => {
					if(res){
            require.ensure([], () => {
          const { export_json_to_excel } = require('@/vendor/Export2Excel')
          const tHeader = ['账目编号', '账目名称','账目分类','金额(元)','出账时间','清账状态'] // 对应表格输出的title
          const filterVal = ['id','name','categoryName','totalAmount','time','status'] // 对应表格输出的数据
          var list = JSON.parse(JSON.stringify(this.list))
            list = res.records
          list.forEach(item => {
            item.status = item.status==3?'已到账':'未到账'
            item.time=this.time_format(item.payDate)
            item.totalAmount=item.totalAmount/100
          });
          const data = this.formatJson(filterVal, list)
          export_json_to_excel(tHeader, data, '出账管理') // 对应下载文件的名字
        })
					}
				})
    ...
    formatJson: function(filterVal, jsonData) {
      return jsonData.map(v => filterVal.map(j => v[j]));
			...
			search:function(currentPage){
			  ...
			  if(currentPage){
				...
				axios_post('/api/fi/inout/list',_self.search_info).then(res => {
					if(res){
      ...
      handleAdd:function(){
        this.$router.push("/finance/fin/addExpenditure")
      },
			handleEdit:function(_id){
			  var _self = this
			  _self.$store.commit('finance_expenditure',{_id:_id})
				_self.$router.push("/finance/fin/expenditureDetail")
			},
      handleDelete:function(_id){
        var _self = this
        _self.$confirm('确认删除该账目？').then(() => {
          axios_post('/api/fi/inout/del/'+_id).then(res => {
