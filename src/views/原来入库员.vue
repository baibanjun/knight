<template>
	<div>
		<h3 class="title_time">时间: {{time}}</h3>
		<el-table :data="list" border style="width: 90%;margin-left: 5%;" show-summary :summary-method="getSummaries">
			<el-table-column type="index" label="#" width="50"></el-table-column>
			<el-table-column label="出入库时间"><template slot-scope="scope"> {{ time_format(scope.row.create_time) }}</template> </el-table-column>
			...
			<el-table-column label="出入库"><template slot-scope="scope"> {{ scope.row.in_out_type==1?'入库':'出库' }}</template> </el-table-column>
			<el-table-column label="类别"><template slot-scope="scope">
          ...
          <el-tab-pane label="4号地磅" name="four" v-if="all_stock_data.four.id">
            <h3 class="tac">4号地磅正称重······</h3>
				...
				<table class="stock_table" cellspacing="0">
					...
					<tr>
						<td>重量: <span style="color:#409EFF;font-size: 20px;padding: 0 10px;">{{stock_data.tempWeight}}kg</span>
					...
					<tr>
						<td>修正后数据</td>
						<td><span style="color:#F56C6C;font-size: 20px;">{{stock_data.fix_weight?(stock_data.fix_weight + 'kg'):''}}</span></td>
					</tr>
				</table>
			</span>
  ...
  import { time_format } from '@/lib/tools.js'
	export default{
        ...
        time:'', //当前时间
				stock_type:0,//进出库
				product_type:0,//货物类型
				all_stock_data:{ //4个磅秤的称重数据
          one:{id:false},two:{id:false},three:{id:false},four:{id:false}
        },
    ...
    computed:{
      stock_status(){
        return this.all_stock_data.one.id?true:this.all_stock_data.two.id?true:this.all_stock_data.three.id?true:this.all_stock_data.four.id?true:false
      },
      stock_data(){
        var _self = this
    ...
    destroyed(){
      ...
      search:function(currentPage){
      	var _self = this
        axios_get('/api/stock',{page:currentPage,limit:10,status:0}).then(res => {
          if(res){
            _self.list = res.list
      ...
      get_stock:function(_index){ //获取磅秤数据
        var _self = this
        axios_get('/api/stock/temp_latest',{deviceId:_self.to_index[_index]}).then(res => {
          if(res){
            if(!res.stockRecord){
      ...
      jump:function(){
        var _self = this
        _self.$confirm('确认跳过本次测量?').then(()=> {
          axios_post_fd('/api/stock/stock_process',{id:_self.stock_data.id,inout_type:3}).then(res => {
            if(res){
              _self.all_stock_data[_self.active_index] = {id:false}
              if(_self.stock_status){
                _self.active_index = _self.all_stock_data.one.id?'one'
                :_self.all_stock_data.two.id?'two'
                :_self.all_stock_data.three.id?'three'
                :_self.all_stock_data.four.id?'four':null
              }
              _self.search(1)
            }
          })
        ...
        const reg = /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/
          ...
          return false
					...
					_self.all_stock_data[_self.active_index].fix_weight = _self.fix_data.value
          _self.all_stock_data[_self.active_index].fix_reason = _self.fix_data.msg
        ...
        _self.$confirm('确认提交本次测量?').then(()=> {
          ...
          //0为出场时
          if(_type==0){
            post_data = {id:_self.stock_data.id,inout_type:_self.stock_data.historyRecord.inOutType,recycle_type:_self.stock_data.historyRecord.recycleType}
          }
          else{
            post_data = {id:_self.stock_data.id,inout_type:_self.stock_type,recycle_type:_type}
          }
          //判断是否有修改
