...
<script>
  ...
  import { time_format } from '@/lib/tools.js'
	export default {
		...
		created() {
      var _self = this
      axios_post('/api/ac/list',{offset:1,limit:100,type:'2',status:'1',flag:'0',}).then(res => {
      	if(res){
		...
		methods:{
			search:function(currentPage){
			  ...
			  if(currentPage){
				...
				axios_post('/api/ca/inout/list',_self.search_info).then(res => {
					if(res){
      ...
      handleAdd:function(){
        this.$router.push("/cashier/addExpenditure")
      },
			handleEdit:function(_id){
			  var _self = this
			  _self.$store.commit('cashier_expenditure',{_id:_id})
				_self.$router.push("/cashier/expenditureDetail")
			},
      handleDelete:function(_id){
        var _self = this
        _self.$confirm('确认删除该账目？').then(() => {
          axios_post('/api/ca/inout/del/'+_id).then(res => {
