...
<script>
	...
	export default{
		...
		computed:{
			statement_id(){
				return this.$store.state.cashier.statement_id
			}
		},
    created() {
      var _self = this
    ...
    mounted() {
      var _self = this
      axios_post('/api/ca/payment/detail/'+_self.statement_id).then(res => {
        if(res){
          _self.info = res
      ...
      handleEdit:function(_type){
        ...
        _self.$refs['ruleForm'].validate((valid) => {
            ...
            if(_type == 1){
              _self.$confirm('确认保存修改信息?', '提示').then(()=> {
                axios_post('/api/ca/payment/save',req).then( res => {
                  if(res===null){
                    _self.$router.push("/cashier/statement")
                  }
                })
              })
            }
              ...
              _self.$confirm('确认已清账?', '提示').then(()=> {
                axios_post('/api/ca/payment/clear',req).then( res => {
                  if(res===null){
                    _self.$router.push("/cashier/statement")
                  }
                })
        ...
        this.$router.push('/cashier/statement')
			},
      //新增和删除扣款项
      add_project:function(){
        var _self = this
        if(this.info.deductions===null) this.info.deductions = []
