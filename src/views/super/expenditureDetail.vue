...
<script>
	...
	export default{
		...
		computed:{
			expenditure_id(){
				return this.$store.state.superUser.expenditure_id
			}
		},
    mounted() {
      var _self = this
      ...
      //该账目详情
      axios_post('/api/sa/inout/detail/'+_self.expenditure_id).then(res => {
        if(res){
      ...
      handleEdit:function(_type){
        ...
        _self.$refs['ruleForm'].validate((valid) => {
            ...
            if(_type == 1){
              _self.$confirm('确认保存修改信息?', '提示').then(()=> {
                axios_post('/api/sa/inout/save',req).then( res => {
                  if(res===null){
                    _self.$router.push("/super/expenditure")
                  }
                })
              })
            }
          ...
          this.$router.push('/super/expenditure')
        })
			}
		}
	}
