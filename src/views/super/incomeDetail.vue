	...
	<div>
      ...
      <el-form class="demo-form-inline" label-width="140px" ref="ruleForm" :rules="rules" :model="info">
              ...
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item label="账目分类:" prop="categoryId">
                    <el-select v-if="info.categoryId!=2&&info.categoryId!=3" v-model="info.categoryId">
                     <el-option v-for="n in categoryList" :label="n.name" :value="n.id"></el-option>
                    </el-select>
                    <el-tag type="info" v-if="info.categoryId==2||info.categoryId==3">油脂售卖</el-tag>
                  </el-form-item>
                </el-col>
              </el-row>
            </td>
        ...
        <el-form-item label="修改备注:">
          ...
          <el-row>
            <el-col :span="4">
              <el-button type="warning" icon="el-icon-document-checked" @click="handleEdit(2)" v-if="info.status==2">确认到账</el-button>
...
<script>
	...
	export default{
		...
		computed:{
			income_id(){
				return this.$store.state.superUser.income_id
			}
		},
    mounted() {
      var _self = this
      ...
      //该账目详情
      axios_post('/api/sa/inout/detail/'+_self.income_id).then(res => {
        if(res){
          ...
          var req = JSON.parse(JSON.stringify(_self.info)) //深拷贝
            ...
            if(_type == 1){
              _self.$confirm('确认保存修改信息?', '提示').then(()=> {
                axios_post('/api/sa/inout/save',req).then( res => {
                  if(res===null){
                    _self.$router.push("/super/income")
                  }
                })
              })
            }
            else{
              _self.$confirm('确认账目已到账?', '提示').then(()=> {
                axios_post('/api/sa/inout/clear',req).then( res => {
                  if(res===null){
                    _self.$router.push("/super/income")
                  }
                })
